...
/** Fetches products from every connected database into the catalog. */
export function SyncDatabaseProductsButton({
  domainId,
}: {
  domainId: string;
}) {
  ...
  async function syncProducts() {
    ...
    setError(null);
    try {
      const res = await fetch("/api/products/sync-database", {
        method: "POST",
      ...
      if (body.totalUpserted === 0) {
        setError(
          failures.length > 0
            ...
            : "Nothing imported — no product rows found in the connected databases.",
        );
        return;
      }
  ...
  return (
    <div className="flex flex-col items-end gap-1.5">
      <Button variant="outline" onClick={syncProducts} disabled={busy}>
        {busy ? (
